#!/usr/bin/env python3
"""
    Break orthogroups into sets of orthologs given a tree from OrthoFinder.

    extract_orthosets_from_orthofinder_trees.py --output <output file> --threshold <float>
        <list of strains> <trees file> <Orthogroups.csv> <Orthogroups_UnassignedGenes.csv>
        <distances directory>
"""

import argparse
import csv
import sys

import dendropy

def get_leaf_genes(node, targets=None):
    if node.is_leaf():
        genes = {node.taxon.label}
    else:
        genes = {n.taxon.label for n in node.leaf_nodes()}
    if targets:
        genes = genes.intersection(targets)
    return genes

def get_leaf_strains(node, targets=None):
    return {g.split('_', 1)[0] for g in get_leaf_genes(node, targets)}

def get_max_dist(node, target_genes):
    max_dist = 0
    accounted_for = set()
    leaves = get_leaf_genes(node, target_genes)
    for l in leaves:
        for l2 in leaves:
            d = dists[(l, l2)]
            if d > max_dist:
                max_dist = d
    return max_dist

parser = argparse.ArgumentParser(usage=__doc__)
parser.add_argument('--output')
parser.add_argument('--threshold', type=float)
parser.add_argument('strainlist')
parser.add_argument('trees')
parser.add_argument('orthogroups')
parser.add_argument('unassigned')
parser.add_argument('dist_dir')
args = parser.parse_args()

dist_threshold = args.threshold

## List of strains to include
focal_strains = set()
with open(args.strainlist, 'rt') as ih:
    for line in ih:
        focal_strains.add(line.strip())

## Full list of orthogroups
orthos = {}
with open(args.orthogroups, 'rt') as ih:
    rdr = csv.DictReader(ih, delimiter='\t')
    for row in rdr:
        OG = row['']
        orthos[OG] = set()
        for strain, genes in row.items():
            if strain == '':
                continue
            if genes == '':
                continue
            for g in genes.split(', '):
                orthos[OG].add(strain + '_' + g)
with open(args.unassigned, 'rt') as ih:
    rdr = csv.DictReader(ih, delimiter='\t')
    for row in rdr:
        OG = row['']
        orthos[OG] = set()
        for strain, genes in row.items():
            if strain == '':
                continue
            if genes == '':
                continue
            for g in genes.split(', '):
                orthos[OG].add(strain + '_' + g)

## Loop over trees
OGs_done = set()
with open(args.output, 'wt') as oh:
    oh.write('orthogroup\tsubset\tsubsubset\tsingle_copy\tn_strains\tcore\tstrains\tgenes\n')
    with open(args.trees, 'rt') as ih:
        for row in ih:
            OG, tree_string = row.strip().split('\t')
            tree = dendropy.Tree.get(data=tree_string, schema='newick',
                                     rooting='default-rooted',
                                     preserve_underscores=True)

            ## List all strains and genes in this tree
            all_genes = set()
            all_strains = set()
            for l in tree.leaf_node_iter():
                name = l.taxon.label
                all_genes.add(name)
                all_strains.add(name.split('_', 1)[0])

            ## Check that the genes in the tree are the same as the genes
            ## in the orthogroups file.
            if all_genes != orthos[OG]:
                raise Exception('Tree does not match Orthogroups.csv for %s' % OG)

            ## Set of focal strains that are actually in this tree --
            ## these are the strains and genes we want to make sure we
            ## include.
            target_strains = set.intersection(all_strains, focal_strains)
            target_genes = {g for g in all_genes if g.split('_', 1)[0] in focal_strains}
            if len(target_genes) == 0:
                continue

            dists = {}
            with open(args.dist_dir + '/' + OG + '.dists', 'rt') as dh:
                dh.readline()
                lines = [l.strip().split() for l in dh]
                gene_names = [l[0] for l in lines if len(l) > 0]
                for i, line in enumerate(lines):
                    for j, dist in enumerate(line[1:]):
                        dists[(gene_names[i], gene_names[j])] = float(dist)

            ## Traverse the tree, recording clades that contain complete sets
            ## of strains and the siblings of those clades. Move to a lower level
            ## if the strains are too distant from each other.
            potential_orthosets = {}
            node_nums = {}
            for i, node in enumerate(tree.preorder_node_iter()):
                node_nums[str(node)] = i
                if target_strains.issubset(get_leaf_strains(node, target_genes)):
                    potential_orthosets[str(node)] = get_leaf_genes(node, target_genes)
                    for n in node.sibling_nodes():
                        g = get_leaf_genes(n, target_genes)
                        if len(g) > 0:
                            potential_orthosets[str(n)] = g

            ## Eliminate redundant sets
            remove = set()
            for nn1, genes1 in potential_orthosets.items():
                i = node_nums[nn1]
                for nn2, genes2 in potential_orthosets.items():
                    j = node_nums[nn2]
                    if i == j:
                        continue
                    if genes1 == genes2:
                        remove.add(min(i, j))
                    elif genes1.issubset(genes2):
                        remove.add(j)
            
            ## Remove gene sets
            for i in remove:
                for nn, j in node_nums.items():
                    if i == j:
                        del potential_orthosets[nn]

            ## Traverse the tree again, chopping up the subsets if they
            ## are too distant.
            final_orthosets = {}
            original_subset_map = {}
            for nn, genes in potential_orthosets.items():
                node = tree.find_node(lambda x: str(x) == nn)
                if node.is_leaf():
                    final_orthosets[nn] = genes
                    original_subset_map[nn] = node_nums[nn]
                else:
                    max_dist = get_max_dist(node, target_genes)
                    if max_dist <= dist_threshold:
                        final_orthosets[nn] = genes
                        original_subset_map[nn] = node_nums[nn]
                    else:
                        accounted_for = set()
                        for n in node.preorder_iter():
                            genes = get_leaf_genes(n, target_genes)
                            if len(accounted_for.intersection(genes)) == 0:
                                if n.is_leaf() or get_max_dist(n, target_genes) <= dist_threshold:
                                    final_orthosets[str(n)] = genes
                                    original_subset_map[str(n)] = node_nums[nn]
                                    accounted_for.update(genes)

            ## Write output
            for nn, genes in final_orthosets.items():
                i = node_nums[nn]
                j = original_subset_map[nn]
                gg = []
                strains_in_set = set()
                for g in genes:
                    s = g.split('_', 1)[0]
                    if s in focal_strains:
                        strains_in_set.add(s)
                        gg.append(g)
                if len(strains_in_set) == 0:
                    continue

                oh.writelines([OG, '\t', OG + '.' + str(j), '\t',
                               OG + '.' + str(i), '\t',
                               str(int(len(gg) == len(strains_in_set))), '\t',
                               str(len(strains_in_set)), '\t',
                               str(int(len(strains_in_set) == len(focal_strains))), '\t',
                               ','.join(sorted(strains_in_set)), '\t',
                               ','.join(sorted(gg)), '\n'])
            OGs_done.add(OG)

    ## Write information for remaining orthogroups -- should be the
    ## singleton "groups" only.
    for OG, genes in orthos.items():
        i = 0
        if OG in OGs_done:
            continue
        gg = []
        strains_in_set = set()
        for g in genes:
            s = g.split('_', 1)[0]
            if s in focal_strains:
                strains_in_set.add(s)
                gg.append(g)
        if len(strains_in_set) == 0:
            continue
        oh.writelines([OG, '\t', OG + '.' + str(i), '\t',
                       OG + '.' + str(i), '\t',
                       str(int(len(gg) == len(strains_in_set))), '\t',
                       str(len(strains_in_set)), '\t',
                       str(int(len(strains_in_set) == len(focal_strains))), '\t',
                       ','.join(sorted(strains_in_set)), '\t',
                       ','.join(sorted(gg)), '\n'])
