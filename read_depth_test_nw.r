#!/usr/bin/env Rscript
#
# Use linear regression to test for differences in read depth
# among treatments.
#
#   read_depth_test_nw.r <file with group 1 input files>
...
cat('contig\tstart\tend\tgene\tp\teffect\trpkm1\trpkm2\tcount1\tcount2\n', file=stdout())
for(i in 1:n_genes) {
    ...
    cat(paste(contig, start_pos, end_pos, gene, p, effect, sep='\t'),
        ...
        '\t', paste(round(raw_counts[trtmnt == 'a'], 3), collapse=','),
        '\t', paste(round(raw_counts[trtmnt == 'b'], 3), collapse=','),
        '\n', sep='', file=stdout())
}
